...
class get_physical_pages(taskmods.DllList):
    ...
    def calculate(self):
        ...
        # get_available_pages== class: volatility.plugins.addrspaces.intel.IA32PagedMemory.get_available_pages
        ...
        with open(PAGES_OUTPUT_PATH + 'pages.' + image_name, 'w') as output:
            ...
            for addr in list_addr:
                ...
                list_paddr.append(physical_addr)

                output.write(str(addr) + '\t' + str(physical_addr) + '\t' + str(size) + '\n')
        self.log('Finish')

        list_paddr.sort()
        print (list_paddr[-1])  # max(paddr) = 4169138176
