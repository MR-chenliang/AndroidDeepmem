...
class IA32PagedMemory(paged.AbstractWritablePagedMemory):
    ...
    def get_available_pages(self, with_pte = False):
        pgd_curr = self.dtb # pgd_curr = 1593344
        for i in range(0, ptrs_per_pgd): # ptrs_per_pgd = ptrs_per_pte = 1024
            start = (i * ptrs_per_pgd * ptrs_per_pte * 4) # i*4k pgd:Page Global Directory /pte:page table entry
            entry = self.read_long_phys(pgd_curr)
            pgd_curr = pgd_curr + 4 #+1byte?
            if self.entry_present(entry) and self.page_size_flag(entry):
                if with_pte:
                    ...
                    #zhixing4ci
                    yield (start, 0x400000) # 0x400000 = 4194304
                    #print (start)
                ...
                for j in range(0, ptrs_per_pte):
                    ...
                    if self.entry_present(pte_entry):
                        if with_pte:
                            ...
                            yield (start + j * 0x1000, 0x1000)#0x1000 == 4096
