...
class AMD64PagedMemory(paged.AbstractWritablePagedMemory):
    ...
    def get_available_pages(self, with_pte = False):
        ...
        #print(type(self.base))  # class 'volatility.plugins.addrspaces.lime.LimeAddressSpace'
        #len(pml4)=4096
        # for i in pml4:
        #     print (i)
        if pml4 is None:
        ...
        #print (pml4_entries)
        for pml4e in range(0, 0x200):  # 0x200 = 512,pml4e = 0,1,2...512
            vaddr = pml4e << 39
            ...
            pdpt = self.base.read(pdpt_base, 0x200 * 8) # 0x200 = 512
            if pdpt is None:
                continue

            pdpt_entries = struct.unpack('<512Q', pdpt)
            for pdpte in range(0, 0x200):   # 0x200 = 512
                ...
                prev_pd_entry = None
                for j in range(0, 0x200): # 512
                    soffset = (j * 0x200 * 0x200 * 8)  # j*2097152
                    ...
                    if self.entry_present(entry) and self.page_size_flag(entry):
                        if with_pte: 
                            ...
                            yield (vaddr + soffset, 0x200000)  # 0x200000 = 2097152
                        ...
                        for k in range(0, 0x200):
                            ...
                            if self.entry_present(pt_entry):
                                if with_pte:
                                    ...
                                    yield (vaddr + soffset + k * 0x1000, 0x1000) # 0x1000=4096
...
class LinuxAMD64PagedMemory(AMD64PagedMemory):#jicheng!!!!!
